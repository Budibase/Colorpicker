...
const PROXYQUIRE_MODULE = '../lib/svenchify.proxyquire.js'
const REQUIRE_MODULE = '../lib/svenchify.require.js'
...
export default (createPluginParts, customizeConfig, finalizeConfig) => {
  const doSvenchify = async (
    ...
    const getConfig = async (...args) => {
      ...
      customizeConfig(config, parts, { wrapSvelteConfig })
  ...
  const svenchify = (...args) => {
    ...
    const svenchifyOptions = parseSvenchifyOptions(options)
